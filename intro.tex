...
We propose a scenario to further emphasize and justify the motivation: Ben is an engineer for 3DSense, a company designing and manufacturing 3D capturing systems and softwares. It is Ben's job to design 3D rigs and associated softwares for 3D capturing tasks. He starts by looking into general computer vision libraries and existing 3D reconstruction techniques, and comes across a general vision library called OpenCV, a multi-view stereo software named PMVS, and a technique called Structured Light. Since he fails to find any out-of-box 3D reconstruction algorithms implemented in OpenCV, he decides to try out PMVS instead. He builds a multi-view camera rig with sufficient overlapping between neighbouring cameras. More specifically, the rig consists of three rings of cameras, each positioned at a zenith angle of $15^\circ$, $45^\circ$, and $75^\circ$. The azimuth angle between two neighbouring cameras is $30^\circ$. He writes a software based on PMVS and provides an interface for parameter settings to fine tune the algorithm, including \underline{window size}, \underline{cell size}, \underline{photo-consistency measure}, and seven other parameter of which the effects on algorithm performance is unclear. Since he has no experience in this field, he decides to use the default parameters. He tests the camera rig and software using a \textit{white matte statue}. The result turns out to be very poor in the sense that the surface is not smooth and contains many unconstructed holes. He starts to try different sets of parameters. He tries to increase the cell size, which leads to even sparser results. After multiple `trial-and-error', the software still fails to achieve a successful result.

Ben comes acorss another library called OpenVL, which provides an interface that allows the user to describe the appearance of an object (called \textit{problem condition}), from which the built-in interpreter selects a reliable algorithm from a suite of algorithms across different categories. The current implementation of the library exposes four parameters to control the visual properties of an object. There are three classes of algorithms implemented: MVS, PS, SL, and VH. Ben builds a multi-purpose camera-project-light rig that is capable of capturing images for all purposes. The API expose four parameters that require user inputs: texture, albedo, specularity, and roughness.

Daisy bought the product of 3DSense, she wants to reconstruct a plaster statue. She uses the simulation software to estimate the property: she set the specularity to 0.2 since the surface is not glossy and kept the rest parameters fixed. A MVS algorithm is selected and the reconstruction result is noisy and full of unconstructed holes. She started to fine tune each parameter, setting texture to 0.2, albedo to 0.8 and roughness to 0.2, which results in a synthetic object that mimicks the real-world object the most. The interpreted algorithm is GSL and the reconstruction result is successful.
