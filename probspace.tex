...
% \label{sec:prob_cond}
Built upon the previously defined problem space, and additional assumptions, we select four of the six properties, the range of which that we are interested in is represented in green, as shown in Figure~\ref{fig:prob_cond} (a). Further, we define four classes of problem conditions that will be investigated in depth, as shown in Figure~\ref{fig:prob_cond} (b). We will demonstrate that it is achievable to design a descritpion-based interface that hides algorithm details and return solutions without knowing the underlying algorithms using objects that satisfy these conditions.
\begin{figure*}[!htbp]
\centering
\begin{tabular}{cc}
\includegraphics[width=0.6\textwidth]{prob_space/prob_cond_1} &
\includegraphics[width=0.3\textwidth]{prob_space/four_class} \\
(a). selected properties & (b). four problem conditions
\end{tabular}
\caption{(a). Most real-world objects do no exhibit extreme properties, such as $0\%$ or $100\%$ brightness, thus we consider the end points as extremities, and represented in grey. The area represented by green is the range of property that we are interested in; (b). Four problem conditions selected based on the definition of problem space and additional assumptions. The description-based interface will be evaluated using objects satisfying these conditions.}
\label{fig:prob_cond}
\end{figure*}
