...
This evaluation intends to demonstrate the robustness of the proof of concept interpreter by demonstrating that: 
...
\textbf{The proof of concept interpreter can return a satisfactory reconstruction given the correct description.}
...
For each object, we will provide a correct description of this object, and see if the algorithm selected by the interpreter will give an acceptable reconstruction result. The acceptance is determiend by visually comparing the result to the baseline method. Thus, the normal map estimated from the PS methods are integrated to get a height map.
...
However, if the interpreter relies solely on the mapping, it is possible that a user-specified description has more then one corresponding algorithm available. We propose to use two additional constraints so that only one appropriate algorithm is selected.

The first constraint is metric-first or shape-first. There are generally two groups of reconstruction results: euclidean/metric reconstruction, and shape reconstruction. The former reconstructs a scene with true metric information, but generally gives noisier results. The latter can achieve quality that is only matched by laser scanners but lack the depth information. The default is metric-first constraint.
...
The proposed proof of concept interpreter that consists of two components, mapping and constraints, is shown in Figure~\ref{fig:interpreter}. Note that there are many ways of using this information to create an interpreter and by no means, is the proposed on the optimal interpreter. 
\begin{figure}[!htbp]
\centering
...
Each object in the synthetic dataset represents one of the four problem conditions discussed in Section~\ref{ch:3DRecon_ProbSpace}. The descriptions of problem conditions of synthetic objects are shown in Table~\ref{tab:synth_prop_list}, as well as the appropriate algorithm selected by the interpreter. Given a user specified description, the proof of concept interpreter will select an algorithm, and any object that matches this description should be well reconstructed by this selected algorithm. Since a description could be mapped to multiple algorithms, an object that does not match the description could potentially have a successful reconstruction result as well. The reconstruction results of synthetic objects and those of the baseline method are shown in Figure~\ref{fig:synth_results}.
\begin{table}[!htbp]
  \centering
  \begin{tabular}{lllllll}
  \hline
  \textbf{Desc\#} & \textbf{Object} & Texture & Albedo & Spec & Rough & Interp Algo\\
  \hline
  1 & bust & 0.2 & 0.8 & 0.2 & 0.8 & GSL\\ % EPS, 
  2 & vase1 & 0.2 & 0.8 & 0.8 & 0.2 & EPS\\
  3 & barrel & 0.8 & 0.8 & 0.2 & 0.8 & GSL\\ % PMVS, EPS, 
  4 & vase0 & 0.8 & 0.8 & 0.8 & 0.2 & PMVS\\ % , EPS
  \hline
  \end{tabular}
  \caption{Problem conditions and interpreted algorithm of the synthetic objects. The description matches the problem condition of corresponding object. The last column shows the algorithm selected by the interpreter.}
...
\subsubsection{Description 4: Vase2}
...
\begin{table}[!htbp]
  ...
  \begin{tabular}{lllllll}
  \toprule
  C\# & Object & Texture & Albedo & Specular & Rough & Interp Algo\\
  \midrule
  1 & statue & 0.2 & 0.8 & 0.2 & 0.8 & GSL\\ % EPS, 
  2 & cup & 0.2 & 0.8 & 0.5 & 0.2 & EPS\\ % , GSL
  3 & pot & 0.8 & 0.8 (0.2) & 0.2 & 0.2 & GSL\\ % PMVS, 
  4 & vase & 0.8 & 0.8 (0.2) & 0.5 & 0.2 & PMVS\\
  \bottomrule
  \end{tabular}
  \caption{Problem conditions and interpreted algorithm of the real-world objects. The description matches the problem condition of corresponding object. The last column shows the algorithm selected by the interpreter.}
...
\bottomrule
\end{tabular}
\caption{The evaluation of interpreter using real-world objects. The first column presents the description provided to the interpreter. Description $i$ matches with the problem condition of synthetic object $i$, which is labeled in green rectangle. The last column is the algorithm selected by the interpreter. Since the interpreter would return a successful reconstruction given a description that matches the problem condition, the quality of reconstruction of the labeled objects indicates success/failure of the interpreter.}
